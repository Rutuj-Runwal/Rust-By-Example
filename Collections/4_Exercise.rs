struct City {
    description: String,
    residents: u64,
    // 👉 add a field here for is_coastal: bool
    //
...
fn new_city(residents: u64, is_coastal: bool) -> City {
    if is_coastal {
        ...
        panic!(
            "👉 return a `City` described as a *non-coastal* city of approximately {} residents"
        );
    }
}

fn main() {
    let rustville: City = panic!("👉 call new_city here, with whatever arguments you like!");

    println!("This city can be described as: 👉 print rustville's `description` here.");
