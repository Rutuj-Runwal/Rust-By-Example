fn main() {
    ...
    let (sum_of_nums,nums) = sum(numbers);
    let (product_of_nums,nums1) = product(nums); // 👉 TODO call product() and put answer here
    let average_of_nums = average(nums1); // 👉 TODO call average() and put answer here
